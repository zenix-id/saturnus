...
class ZmsSecret(models.Model):
    _name = 'zms.secret'
    _description = 'ZmsSecret'
    ...
    def create(self, vals_list):
        ...
        try:
            new_secrets = super(ZmsSecret, self).create(processed_vals_list)
        ...
        for secret in new_secrets:
            ...
            try:
                codex_entry_rec = codex_model.assign_codex_to_record('zms.secret', secret.id)
    ...
    def unlink(self):
        ...
        codex_model = self.env['zms.codex']
        model_id = self.env['ir.model']._get_id('zms.secret')
        ...
        else: _logger.warning("Model 'zms.secret' not found. Skipping ZMS Codex cleanup.")
        result = super(ZmsSecret, self).unlink()
    ...
    def copy(self, default=None):
        ...
        new_secret = super(ZmsSecret, self).copy(default=default)
        codex_model = self.env['zms.codex']
        codex_entry_rec = None
        try:
            codex_entry_rec = codex_model.assign_codex_to_record('zms.secret', new_secret.id)
    ...
    def write(self, vals):
        return super(ZmsSecret, self).write(vals)